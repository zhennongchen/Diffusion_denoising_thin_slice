...
for i in range(0,1):#n.shape[0]):
    ...
    ddpm_avg_files = ff.sort_timeframe(ff.find_all_target_files(['pred_img_scans*'], os.path.join('/mnt/camca_NAS/denoising/models/unsupervised_DDPM_gaussian_2D/pred_images', patient_id, patient_subid,'random_'+str(random_n), 'epoch70avg')),2,'s','.')
    ...
    for k in range(0,len(ddpm_avg_files)):
        ...
        ddpm_avg_img_brain = Data_processing.cutoff_intensity(ddpm_avg_img, cutoff_low=-100, cutoff_high=100)
        mae_brain_ddpm_avg, _, rmse_brain_ddpm_avg, _, ssim_brain_ddpm_avg,psnr_brain_ddpm_avg = ff.compare(ddpm_avg_img_brain, gt_img_brain, cutoff_low = 0, cutoff_high = 100)
        mae_list.append(mae_brain_ddpm_avg)
        rmse_list.append(rmse_brain_ddpm_avg)
        ssim_list.append(ssim_brain_ddpm_avg)
